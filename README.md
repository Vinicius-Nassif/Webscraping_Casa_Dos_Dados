...
Após a obtenção das informações de todos os registros, esses dados serão inseridos e armazenados em uma planilha do Excel (.xlsx), bem como no MySQL.

O Intuito dessa atividade irá expor mais intensivamente a automação web, loops, variáveis, manipulação de arquivos, divisão de tarefas em módulos, tratamento de exceções e criação de um banco de dados no MySQL, conforme será demonstrado a seguir.
